...
TH1D* GetSmearedSpecial(TH1D *hist)
{
  ...
  for (int i = 1; i <= Nbins; i++) {
    ...
    double rand2 = grand -> Gaus(-0.25, 0.25);
    // size of the main shift:
    double corr = 0.90;
    double sign = -1;
    // flip the correlation sigh over bins, see the sign of the measure correlations as a cross-check all works fine;)
    for (int k = 0; k < i;++k)
      sign *= -1;
    toy -> SetBinContent(i, hist->GetBinContent(i) + sign*i*(rand+rand2)*corr*hist->GetBinError(i));
    toy -> SetBinError(i, hist->GetBinError(i));
  }
  toy -> Scale(1.);
  return toy;
...
// initialize the class with number of kinematic bins (say top pT bins), and some binning and limits for storage of the internal cross sections in each bin you provide in each toy
  ...
  TCanvas *can2 = new TCanvas("master2", "master2", 800, 400, 800, 600);
  ...
  can1 -> Print(TString(can1->GetName())+".pdf");
  can2 -> Print(TString(can1->GetName())+".pdf");
