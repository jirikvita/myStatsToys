...
void ToyCovariance::Init()
{
  ...
  for (unsigned int i = 0; i < _Nbins; ++i) {
	...
	vecprod.push_back(new TH1D(name.c_str(), name.c_str(), _nHistBins, (_Zmin < 0 ? -1. : 1.)*_Zmin*_Zmin,  (_Zmax < 0 ? -1. : 1.)*_Zmax*_Zmax));
      ...
      if (_mode != 0)
	veccorr.push_back(new TH2D(name.c_str(), name.c_str(), 2*_nHistBins, -_Zmax, _Zmax, _nHistBins, -_Zmax, _Zmax));
      else
	veccorr.push_back(new TH2D(name.c_str(), name.c_str(), 2*_nHistBins, _Zmin, _Zmax, _nHistBins, _Zmin, _Zmax));
    }
