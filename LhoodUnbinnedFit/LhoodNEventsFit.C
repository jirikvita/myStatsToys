...
// 8 Feb 2018
//
// ideas: compare also to simple fit of generated s+b to the s+b fit function?;-)
//
// running:
// root -l LhoodNEventsFit.C+
// 
...
// __________________________________________
// compute the lhood from the shape of the expected mtop distribution
...
double GetShapeLhoodLog(TF1 *bgshape, TF1 *sigshape)
{
  ...
  // go through the generated data as function of top mass:
...
double GetNegLhoodLog(Double_t *par)
{
  ...
  if (gCall < maxNCalls) {
    for (int i = 0; i < gnPars; ++i) {
      ...
      if (ghistosReady)
	gGrLHvspar[i] -> SetPoint(gGrLHvspar[i]->GetN() + 1,par[i], lhood);
    }
  }
...

{
  ...
  gGrLHvspar = new TGraph*[nPars];  

  assert(nPars < maxNpars);
  TString parname[nPars] = {"N_{sig}", "N_{bg}", "m_{t}"};
  double parmax[nPars] = {Nevts + deltaNevts, Nevts + deltaNevts, gtopMax};
  double parmin[nPars] = {0, 0, gtopMin};
  ...
  */
  ...
  // test the rough lhood returned range for histogram axes:
  ...
  // manual walk over the grid of pars:
  ...
  for (int i = 0; i < nPars; ++i) {
    gHparevolution[i] = new TH1D(parname[i], (parname[i] + ";icall;" + parname[i]), maxNCalls, 0, maxNCalls);
    // this histo for axes only:
    gHLHvspar[i] = new TH2D(("-log(L) vs " + parname[i]), ("-log(L) vs " + parname[i] + ";" + parname[i]), 
    			    10*nbins, parmin[i], parmax[i], nbins, gLogLHmin, gLogLHmax);
    // graph fpr data points:
    gGrLHvspar[i] = new TGraph();
    gGrLHvspar[i] -> SetMarkerStyle(7);
    gGrLHvspar[i] -> SetMarkerColor(kBlack);
    gGrLHvspar[i] -> SetMarkerSize(1);
  ...
  step[2] = 0.5;
  for (int i = 0; i < nPars; ++i) {
    gMinuit->mnparm(i, parname[i].Data(), vstart[i], step[i], parmin[i], parmax[i], ierflg);

  }
  /*  gMinuit->mnparm(0, parname[0].Data(), vstart[0], step[0], 0, Nevts + deltaNevts, ierflg);
      gMinuit->mnparm(1, parname[1].Data(),  vstart[1], step[1], 0, Nevts + deltaNevts,ierflg);
      gMinuit->mnparm(2, parname[2].Data(), vstart[2], step[2], topMin, topMax, ierflg);
  */
  ...
  // Print results
   ...
   // visualisation of the shape of the lhood as function of top mass
   cout << "Visualising the Lhood profiles, generating TGraphs, stay calm..." << endl;

   ntestSteps = 100;
   int ix = 2;
   TGraph *gr_mtop = new TGraph();
   gr_mtop -> SetName("logLhood");
   gr_mtop -> SetMarkerStyle(20);
   gr_mtop -> SetMarkerColor(kBlue);
   gr_mtop -> SetMarkerSize(1);
   for (int k = 0; k < ntestSteps; ++k) {
     for (int ipar = 0; ipar < nPars; ++ipar)
       testpars[ipar] = FitPars[ipar];
     testpars[ix] = testparmin[ix] + k*(testparmax[ix] - testparmin[ix]) / (ntestSteps);
     lhood = GetNegLhoodLog(testpars);
     gr_mtop -> SetPoint(k, testpars[ix], lhood);
   }
   ...
   gr2d -> SetName(TString("lhoodProf"));
   gr2d -> SetMarkerStyle(7);
   gr2d -> SetMarkerColor(kBlack);
   gr2d -> SetMarkerSize(2);

   for (int k = 0; k < ntestSteps; ++k) {
     for (int ipar = 0; ipar < nPars; ++ipar)
       testpars[ipar] = FitPars[ipar];
     testpars[ix] = testparmin[ix] + k*(testparmax[ix] - testparmin[ix]) / (ntestSteps);
     for (int l = 0; l < ntestSteps; ++l) {
       testpars[iy] = testparmin[iy] + l*(testparmax[iy] - testparmin[iy]) / (ntestSteps);
       lhood = GetNegLhoodLog(testpars);
       gr2d -> SetPoint(ip, testpars[ix], testpars[iy], lhood);
       ip++;
     }
   }

   TString name = "lhoodProf";
   TCanvas *lhoodProfCan = new TCanvas(name, name, 100, 100, 1600, 800);
   lhoodProfCan -> Divide(2,1);

   lhoodProfCan -> cd(1);
   gr_mtop -> Draw("AP") ;
   gr_mtop -> SetTitle(TString("lhood1D;") + parname[ix] + ";-ln L");
   // gr_mtop -> GetHistogram() -> GetXaxis() -> SetTitle(parname[ix]);
   // gr_mtop -> GetHistogram() -> GetYaxis() -> SetTitle("-ln L");
   gPad->Update();
   
   lhoodProfCan -> cd(2);
   gr2d -> Draw("surf1");
   gr2d -> SetTitle(TString("lhood2D;") + parname[ix] + ";" + parname[iy] + ";-ln L");
   //   gr2d -> GetHistogram() -> GetXaxis() -> SetTitle(parname[ix]);
   //   gr2d -> GetHistogram() -> GetYaxis() -> SetTitle(parname[iy]);
   //   gr2d -> GetHistogram() -> GetZaxis() -> SetTitle("-ln L");
   gPad->Update();

   lhoodProfCan -> Print(TString(can -> GetName()) + ".png");
   lhoodProfCan -> Print(TString(can -> GetName()) + ".pdf");
